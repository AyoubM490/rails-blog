...
def create_and_activate_likes(author, post, number_of_likes)
...
def create_and_activate_comments(author, post, number_of_comments)
  number_of_comments.downto(1).each do |index|
    Comment.create!(author: "author", post: "some post", text: "Comment #{index} made by #{author.name} on post #{post.id}")
  end
end
